...
use crate::commands::{check_for_updates_blocking, UpdateInfo};
    ...
    thread::spawn(move || loop {
        match check_for_updates_blocking() {
            Ok(info) => {
                ...
                // Always emit so a frontend that joined mid-session
                // (e.g. after window reload) renders the latest state.
                if let Err(e) = handle.emit(UPDATE_EVENT, &info) {
                    eprintln!("[launcher] emit update-available failed: {e}");
                }

                // Rising edge: show the in-window toast. Falling edge:
                // remove any existing toast (covers the "user just
                // applied the update" → next poll → update_available is
                // false → tear down the toast) case.
                if !was && now {
            ...
            Err(e) => {
                // Network hiccup, offline, GitHub down — log and try again
                // next cycle. Not user-facing; an update check that can't
                // resolve isn't worth a scary error toast.
                eprintln!("[launcher] update check failed: {e}");
            }
        }
